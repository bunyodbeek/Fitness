...
class HandbookItem(CreatedBaseModel):
    category = models.ForeignKey(
        ...
        blank=True, null=True,
        verbose_name=_("Kategoriya (to'g'ridan)")
    )
    subcategory = models.ForeignKey(
        ...
        related_name='items',
        blank=True, null=True,
        verbose_name=_("Sub-kategoriya")
    )
    title = models.CharField(max_length=300, verbose_name=_("Nomi"))
    title_uz = models.CharField(max_length=300, blank=True, null=True)
    ...
    short_description_uz = models.TextField(max_length=500, blank=True, null=True)
    short_description_ru = models.TextField(max_length=500, blank=True, null=True)
    short_description_en = models.TextField(max_length=500, blank=True, null=True)
    ...
    class Meta:
        ...
        constraints = [
            ...
            models.UniqueConstraint(
                ...
                name='uq_handbook_item_slug_per_category',
                condition=Q(category__isnull=False, subcategory__isnull=True)
            ),
        ]
    ...
    def clean(self):
        ...
        if not self.subcategory and not self.category:
            raise ValidationError("Subcategory yoki Category bo'lishi shart!")
